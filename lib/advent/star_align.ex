defmodule Advent.StarAlign do
  ...
  def parse(star) do
    [x, y, j, k] =
      Regex.run(~r{\Aposition=<\s*(-?\d+), \s*(-?\d+)>\s+velocity=<\s*(-?\d+), \s*(-?\d+)>\z}, star, capture: :all_but_first)
  ...
  def move_stars(stars) do
    stars
    |> Enum.map(fn star ->
      %{star | position: %{x: star.position.x + star.velocity.x, y: star.position.y + star.velocity.y }}
    end)
  end
